      ...
      <AnimatePresence>
        {completedPoints !== null && (
          <motion.div initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }} style={{ position: 'fixed', bottom: '2rem', right: '2rem', zIndex: 9999, display: 'flex', alignItems: 'center', gap: '1rem', background: '#1e293b', border: '1px solid rgba(88, 101, 242, 0.3)', borderRadius: '16px', padding: '1.2rem 1.5rem', boxShadow: '0 10px 40px -10px rgba(0, 0, 0, 0.5)' }}>
            <div style={{ background: 'rgba(88, 101, 242, 0.1)', color: '#5865F2', padding: '0.6rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <CheckCircle size={24} />
            </div>
            <div>
              <h4 style={{ color: 'var(--foreground)', fontSize: '1rem', fontWeight: 700, margin: 0, marginBottom: '0.2rem' }}>Task Completed!</h4>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', margin: 0 }}>You earned <span style={{ color: '#5865F2', fontWeight: 800 }}>+{completedPoints}</span> points.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} style={{ background: 'var(--background)', border: '1px solid var(--glass-border)', borderRadius: '20px', width: '100%', maxWidth: '500px', padding: '2rem' }}>
              ...
              <form onSubmit={handleCreateTask} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <input type="text" required value={title} onChange={e => setTitle(e.target.value)} style={{ width: '100%', padding: '0.8rem', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', borderRadius: '10px', color: 'var(--foreground)' }} placeholder="e.g. Moderate Voice Channel" />
                </div>
